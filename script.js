...
function initAddPage() {
    ...
    addBtn.addEventListener('click', async () => {
        ...
        try {
            // 기존 일정은 건드리지 않고 새 일정만 추가
            await db.collection('trips')
                .doc(String(newTrip.id))
                .set(newTrip);

            console.log('새 일정 저장 완료:', newTrip);

            window.location.href = 'index.html';

        } catch (error) {
            console.error('새 일정 저장 실패:', error);
    ...
    try {
        console.log('🔥 loadTrips 시작');
        console.log('🔥 db:', window.db);

        const snapshot = await window.db.collection('trips').get();

        console.log('🔥 Firestore 문서 개수:', snapshot.size);

        trips = snapshot.docs.map(doc => {
            const data = doc.data();

            console.log('🔥 문서:', doc.id, data);
...
function initAddPage() {
    ...
    try {
        ...
        alert("Firestore에서 여행 일정을 불러오지 못했습니다.");
