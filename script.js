if (!window.travelPlannerInitialized) {
    ...
    document.addEventListener('DOMContentLoaded', async () => {
        ...
        if (!db) {
            console.error('Firebase Firestore 연결 실패');
        ...
        function initIndexPage() {
            ...
            if (notificationBtn && notificationDropdown && notificationBtn.dataset.initialized !== 'true') {
                ...
                notificationBtn.addEventListener('click', event => {
                    ...
                    notificationDropdown.style.zIndex = '10000';
                });
            ...
            function renderNotifications() {
                ...
                upcomingTrips.forEach(trip => {
                    const startDate = parseLocalDate(trip.startDate);
                    const dDay = Math.round((startDate.getTime() - today.getTime()) / (1000 * 60 * 60 * 24));
                    ...
                    if (dDay === 0) {
                        li.innerHTML = `
                            <div class="noti-content">
                                <span class="d-day-badge d-day-today">TODAY</span>
                                <div class="noti-text">
                    ...
                    li.addEventListener('click', () => {
                        window.location.href = `trip_detail.html?id=${encodeURIComponent(String(trip.id))}`;
                    });
            ...
            function performSearch() {
                ...
                const filtered = trips.filter(trip => {
                    ...
                    const nameMatch = (trip.destination || '').toLowerCase().includes(term);
                ...
                filtered.forEach(trip => {
                    ...
                    card.innerHTML = `
                        ...
                        <div class="trip-card-actions">
                            <button class="edit-trip-btn" type="button" title="여행 정보 수정">
                                <i class="fa-solid fa-pen"></i>
                            </button>
                            <button class="delete-trip-btn" type="button" title="여행 삭제">
        ...
        async function initDetailPage() {
            ...
            if (title) {
                title.textContent = currentTrip.destination;
            }

            if (dates) {
                dates.textContent = `${currentTrip.startDate} ~ ${currentTrip.endDate}`;
            }
            ...
            if (placesList && placesList.dataset.initialized !== 'true') {
                ...
                placesList.addEventListener('dragover', event => {
                    ...
                    const after = getDragAfterElement(placesList, event.clientY);
            ...
            function getDragAfterElement(container, y) {
                const elements = [...container.querySelectorAll('.place-item:not(.dragging)')];
            ...
            async function updateOrder() {
                ...
                const ids = [...items].map(item => String(item.dataset.id));
                const reordered = [];

                ids.forEach(id => {
                    const found = currentTrip.places.find(
                        place => String(place.id) === id
                    );
            ...
            async function optimizeRoute() {
                ...
                const unlocked = places.filter(place => !place.isLocked);
                ...
                while (unlockedCopy.length > 0) {
                    ...
                    for (let i = 0; i < unlockedCopy.length; i++) {
                        const distance = calculateDistance(current, unlockedCopy[i]);
        ...
        function initIndexPage() {
            ...
            function performSearch() {
                ...
                const filtered = trips.filter(trip => {
                    ...
                    const nameMatch = (trip.destination || '')
                        .toLowerCase()
                        .includes(term);
